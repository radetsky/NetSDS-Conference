...
if ( @phones == 0 ) {
    my $out = sprintf $error,
      "Не указано ни одного номера телефона";
    print $out, "\n";
    exit(0);
}


# Проверка назначения прав операторов. 
$adm{'oper'} = 0;

if ( exists $params{'op_rights'} and $params{'op_rights'} eq "on" ) {
    unless ($admin) {
        my $out = sprintf $error,
"Назначать операторов имеет право только администратор";
        print $out, "\n";
        exit(0);
    }
    $adm{'oper'}   = 1;
...
# Если пользователь не новый, но загружаем его параметры
my %old = ();
if ( $user{'id'} ne 'new' ) {
